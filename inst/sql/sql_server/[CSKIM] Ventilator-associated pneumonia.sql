...
SELECT 38 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
                                            ( 
                                              select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4302666)
  ...
  JOIN (
    ...
    (
      ...
      (
        ...
        ) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= DATEADD(day,2,P.START_DATE) AND A.visit_occurrence_id = P.visit_occurrence_id
...
ELSE
BEGIN
insert INTO #inclusion_5 select * from #inclusion_0 where 1 = 0
END
;
